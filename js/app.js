...
function employee(fullName, department, level, imageURL, salary, employeeID) {
...
function getData() {
  ...
  if (parseemploeey !== null) {
    ...
    for (let i = 0; i < parseemploeey.length; i++) {
      new employee(parseemploeey[i].fullName, parseemploeey[i].department, parseemploeey[i].level, parseemploeey[i].imageURL, parseemploeey[i].salary, parseemploeey[i].employeeID)
...
function handelsubmit(event) {
  ...
  let newEmployee = new employee(name, department, level, imageURL, 0, 0)
  form.reset();
